...
def app():
    ...
    real_difficulty = st.number_input(
        "Фактическая сложность работы над проектом", min_value=0)
    proj_rate = st.number_input("Оценка проекта", min_value=0)

    add = st.button("Добавить")
    if add:
        staff_database.iloc[emp_n, staff_database.columns.get_loc(
    ...
    if len(staff_database["real_difficulty"].dropna()) == 0:
        ...
        coef = np.mean(np.array(staff_database["estimated_difficulty"].dropna()) / np.array(
            staff_database["real_difficulty"].dropna()))
